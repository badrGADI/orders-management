...
export function OrderForm({ initialData }: OrderFormProps) {
  ...
  const [productOptions, setProductOptions] = useState<{ value: string; label: string; data: Product }[]>([])
  ...
  const form = useForm({
    ...
    validate: {
      ...
      customerEmail: (value) => (/^\S+@\S+$/.test(value) ? null : "Invalid email"),
      ...
      items: {
        ...
        unitPrice: (value) => (value > 0 ? null : "Price must be greater than 0"),
      },
    },
  })
  ...
  const addItem = () => {
    form.insertListItem("items", { productId: "", productName: "", quantity: 1, unitPrice: 0 })
  }
  ...
  const calculateSubtotal = () => {
    return form.values.items.reduce((sum, item) => sum + item.quantity * item.unitPrice, 0)
  }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card withBorder radius="md" p="lg">
            ...
            <Divider my="md" label="Or enter customer details manually" labelPosition="center" />

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
              <TextInput label="Name" placeholder="Customer name" {...form.getInputProps("customerName")} />
              <TextInput label="Email" placeholder="customer@example.com" {...form.getInputProps("customerEmail")} />
              <TextInput label="Phone" placeholder="Phone number" {...form.getInputProps("customerPhone")} />
              <TextInput label="Address" placeholder="Delivery address" {...form.getInputProps("customerAddress")} />
            </div>
          </Card>
          ...
          <Card withBorder radius="md" p="lg">
            <Card.Section withBorder inheritPadding py="xs" mb="md">
              <Group justify="space-between">
                ...
                <Button leftSection={<IconPlus size={16} />} onClick={addItem} size="xs">
                  Add Item
                </Button>
              </Group>
            </Card.Section>
                    ...
                    <Table.Tr key={index}>
                      <Table.Td>
                        <Autocomplete
                          ...
                          data={productOptions}
                          onChange={(query) => {
                            form.setFieldValue(`items.${index}.productName`, query)
                            handleProductSearch(query)
                          }}
                          onOptionSubmit={(value) => handleProductSelect(value, index)}
                          rightSection={productSearchLoading ? <Loader size="xs" /> : null}
                      ...
                      <Table.Td>
                        <NumberInput
                          min={1}
                          value={item.quantity}
                          onChange={(value) => form.setFieldValue(`items.${index}.quantity`, value || 1)}
                      ...
                      <Table.Td>
                        <NumberInput
                          ...
                          onChange={(value) => form.setFieldValue(`items.${index}.unitPrice`, value || 0)}
            ...
            <div className="mt-4 space-y-4">
              ...
              <div>
                <NumberInput
                  ...
                  onChange={(value) => form.setFieldValue("shipping", value || 0)}
                  rightSection="€"
                />
              </div>
